...
'''
# bert   codet5  cotext  gpt  graph  plbart
# Load data
args.model = 'sage'
uuo = 'bert'
where = 'gat/' + uuo
filenames = where
# the range for each test class
ll = 8014
rr = 8092
if args.dataset == 'cora':
    adj, features, labels = data_load.load_data(path='data/'+uuo+'/')
    class_sample_num = 2
    im_class_num = 1
...
if args.dataset == 'BlogCatalog':
    ...
    idx_train, idx_val, idx_test = utils.split_arti(labels, ll, rr)
elif args.dataset == 'twitter':
    idx_train, idx_val, idx_test, class_num_mat = utils.split_genuine(labels)
...
def train(epoch):
    ...
    acc_train, p, r, f1 = utils.accuracy(output[idx_train], labels_new[idx_train])
    ...
    acc_val, p1, r1, f11 = utils.accuracy(output[idx_val], labels[idx_val])
    ...
    print('Epoch: {:05d}'.format(epoch+1),
          ...
          "precision_train= {:.4f}".format(p),
          "recall_train= {:.4f}".format(r),
          "f1-score_train= {:.4f}".format(f1),
...
def test(epoch = 0):
    ...
    acc_test, p, r, f1 = utils.accuracy(output[idx_test], labels[idx_test])
...
def save_model(epoch):
    ...
    torch.save(saved_content, 'checkpoint/cora/'+where+'.pth')

    return

def load_model(filename):
    loaded_content = torch.load('checkpoint/cora/{}.pth'.format(filename), map_location=lambda storage, loc: storage)
    ...
    print("successfully loaded: " + 'checkpoint/cora/{}.pth'.format(filename))

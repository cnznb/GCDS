...
def train_search(path):
    for file_name in os.listdir(path):
        ...
        if file_name == '[test]ganttproject' or file_name == '[test]xerces' or file_name == 'java-algorithms-implementation':
            continue
...
def val_search(path):
    for file_name in os.listdir(path):
        ...
        if os.path.isdir(file_path):
            if file_name.startswith('1'):
                xi = tot-1
                val_dataset.append(solve(file_path))
                print(path, xi, tot-1)
            else:
                val_search(file_path)
...
tot = 1
train_search('E:/datasets')
val_search('E:/datasets/java-algorithms-implementation')

# -*- encoding = utf-8 -*-
"""
@description: Using CodeT5 to generate text vectors for each field in dataset
@date: 2022/9/4
@File : CodeT5.py
@Software : PyCharm
"""
...
model_path = 'D:\\workspace\\EC\\codet5-base-multi-sum' # sys.argv[1]  # CodeT5预训练模型目录
...
def solve(path):
    ...
    with open(path + '\\field_range.csv', 'r') as fp:
        ...
        # if os.path.exists(path + '\\codeEmbedding\\codeT5.csv'):
        #     os.remove(path+'\\codeEmbedding\\codeT5.csv')
        for line in lines:
            if line == '':
                break
            code_line = ''
            ...
            with open(path+'\\codeEmbedding\\codeT5.csv', 'a', newline='') as csv_file:
